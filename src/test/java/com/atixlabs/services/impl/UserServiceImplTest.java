...
import com.atixlabs.exceptions.InexistentUserException;
...
class UserServiceImplTest {
    ...
    void updateValidUser() {
        long userId = 10;
        User user = new User(10L, "oldName", "oldSurname", "oldUsername", "", Role.builder().code(com.atixlabs.model.constants.Role.ROLE_ADMIN.role()).id(1).build());
        Mockito.when(repository.findById(userId)).thenReturn(Optional.of(user));
        User userExpected = new User(10L, "newName", "newSurname", "newUsername@atixlabs.com", "", Role.builder().code(com.atixlabs.model.constants.Role.ROLE_ANALYST.role()).build());
